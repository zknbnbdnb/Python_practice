...
def get_foramtted_name(first_name, last_name, middle_name = None):  ###None是在条件测试中特殊的空字符，仅在分支时使用
    if middle_name:
        fullName = f"{first_name} {middle_name} {last_name}"
    else:
        fullName = f"{first_name} {last_name}"
...
unprinted_designs = ['phone case', 'robot pendant', 'dodecahederon']
completed_models = []
print_models(unprinted_designs[:], completed_models)    ###前面和后面一个的代码虽然很相似，但是这里加了个切片表示法，保留了
...
user_profile = bulid_profile('zhou', 'kang',location = 'shenzhen', field = 'Python', zhiye = 'xueshen')
print(user_profile)
